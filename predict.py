...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        clicks: str = Input(description="List of click coordinates in format '[x,y],[x,y],...'"),
        labels: str = Input(description="List of labels corresponding to clicks, e.g., '1,1,0,1'"),
        affected_frames: str = Input(description="List of frame indices for each click, e.g., '0,0,150,0'"),
        ann_obj_ids: str = Input(description="List of object IDs corresponding to each click, e.g., '1,1,1,2'"),
        vis_frame_stride: int = Input(default=15, description="Stride for visualizing frames"),
    ) -> Output:
        video_dir = self._extract_video_frames(video)
        frame_names = self._get_sorted_frame_names(video_dir)
        ...
        video_segments = self._process_prompts(inference_state, prompts, frame_names, output_dir, video_dir)
        ...
        black_white_masks, highlighted_frames = self._visualize_results(
            video_segments, frame_names, prompts, vis_frame_stride, output_dir, video_dir
        )

        self._cleanup(video_dir)

        return Output(black_white_masks=black_white_masks, highlighted_frames=highlighted_frames)

    def _extract_video_frames(self, video):
    ...
    def _get_sorted_frame_names(self, video_dir):
        ...
        return sorted(frame_names, key=lambda p: int(os.path.splitext(p)[0]))

    def _parse_inputs(self, clicks, labels, affected_frames, ann_obj_ids):
        click_list = [list(map(int, click.split(","))) for click in clicks.strip("[]").split("],[")]
        ...
        if not (len(click_list) == len(label_list) == len(frame_list) == len(obj_id_list)):
            raise ValueError("The number of clicks, labels, affected frames, and object IDs must be the same.")

        prompts = {}
        for click, label, frame, obj_id in zip(click_list, label_list, frame_list, obj_id_list):
            if obj_id not in prompts:
                prompts[obj_id] = []
            prompts[obj_id].append((frame, np.array([click], dtype=np.float32), np.array([label], np.int32)))

        return prompts

    def _process_prompts(self, inference_state, prompts, frame_names, output_dir, video_dir):
        video_segments = {}
        for obj_id, obj_prompts in prompts.items():
            for frame, points, labels in obj_prompts:
                out_obj_ids, out_mask_logits = self.refine_mask(inference_state, frame, obj_id, points, labels)
                
                if frame not in video_segments:
                    video_segments[frame] = {}
                video_segments[frame][obj_id] = out_mask_logits[0].cpu().numpy()

                self._visualize_step(frame, obj_id, points, labels, out_mask_logits, frame_names, output_dir, video_dir)
    ...
    def _propagate_masks(self, inference_state, video_segments):
        for out_frame_idx, out_obj_ids, out_mask_logits in self.predictor.propagate_in_video(inference_state):
            ...
            for i, out_obj_id in enumerate(out_obj_ids):
                video_segments[out_frame_idx][out_obj_id] = out_mask_logits[i].cpu().numpy()
        return video_segments

    def _visualize_results(self, video_segments, frame_names, prompts, vis_frame_stride, output_dir, video_dir):
        black_white_masks = []
        highlighted_frames = []
        for out_frame_idx in range(0, len(frame_names), vis_frame_stride):
            bw_mask, highlighted_frame = self._create_frame_visualization(
                out_frame_idx, video_segments, prompts, frame_names, output_dir, video_dir
            )
    ...
    def _visualize_step(self, frame, obj_id, points, labels, out_mask_logits, frame_names, output_dir, video_dir):
    ...
    def _create_frame_visualization(self, out_frame_idx, video_segments, prompts, frame_names, output_dir, video_dir):
        fig = plt.figure(figsize=(12, 8))
        plt.title(f"frame {out_frame_idx}")
        plt.imshow(Image.open(os.path.join(video_dir, frame_names[out_frame_idx])))

        combined_mask = np.zeros_like(next(iter(video_segments[out_frame_idx].values())).squeeze(), dtype=np.float32)
